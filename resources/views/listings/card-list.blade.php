@foreach ($listings as $key => $listing)
  ...
  <div class="m-5">
    ...
    <div
      ...
      <a data-modal-target="large-modal{{ $listing->id }}"
        data-modal-toggle="large-modal{{ $listing->id }}" class="col-span-2 text-left text-gray-600 hover:text-gray-700">
      ...
      <div class="col-span-3 flex flex-col space-y-3 pr-8 text-left">
        <a href='javascript:void(0)' data-modal-target="large-modal{{ $listing->id }}"
            data-modal-toggle="large-modal{{ $listing->id }}" class="mt-3 invisible sm:visible overflow-hidden text-2xl font-semibold">
          {{ Str::limit($listing->title, 50, '...') }}
        </a>
        <a href='javascript:void(0)' data-modal-target="large-modal{{ $listing->id }}"
            data-modal-toggle="large-modal{{ $listing->id }}" class="mt-3 sm:hidden overflow-hidden text-2xl font-semibold">
